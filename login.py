...
parser.add_argument('-P', dest='bind_port', default=0, type=int,
                    help='api bing port (default: 0) if port is 0, it will not bind')
parser.add_argument('-s', dest='session_filename', default="cookies.json",
                    help='session cookies save fileame (default: cookies.json)')
...
class Spider(object):
    ...
    def get_session(self, force_refresh=False):
        if not force_refresh and self.session and time.time() - int(self.session.get('ts', 0)) < int(
                self.session.get('expires_in', 0)) / 2.0:
            return
...
class SessionRequestHandler(RequestHandler):
    ...
    def get(self):
        ...
        if name:
            ...
            self.write(
                ("&".join(["%s=%s" % (key, value) for key, value in spider.session.iteritems()])).encode("utf-8"))
